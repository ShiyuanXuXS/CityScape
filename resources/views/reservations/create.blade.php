        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="my-6 p-6 bg-white border-b border-gray-200 shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('reservations.store') }}" method="post">
                    ...
                    @if ( $hotel_rooms)
                    <div>
                        <label for="hotel_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a hotel</label>
                        <select id="hotel_name" 
                            value="@old('hotel_name')" 
                            name="hotel_name" 
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        >
                            <option selected>{{ @old('hotel_name')?:"Choose a hotel" }}</option>
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="mx-4">Check out</span>
                        <div class="relative">
                        ...
                        >
                            ...
                            @foreach($hotel_rooms as $hotel)
                                @foreach($hotel["rooms"] as $room)
                                    <option value="{{ $room["room_number"] }}" {{ ($room["room_number"]==@old('room_number'))?"selected": ""}}>Room Number: {{ $room["room_number"] }} --Capacity: {{ $room["room_capacity"] }} --Rate: {{ $room["room_rate"] }}</option>
                                @endforeach
                            @endforeach
                        </select>
                    </div>
                    ...
                    <x-button class="mt-6">Reserve</x-button>
